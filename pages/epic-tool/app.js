...
function setupHtmlEvents () {
  ...
  // const overlayButton = document.getElementById('overlayButton') // 暂时注释
  // overlayButton.addEventListener('click', onExpandOverlay_Click)

  const resizeCheckBox = document.getElementById(
  ...
  if (encoderParamsSubmit !== null) {
    encoderParamsSubmit.onclick = function (event) {
      const rateControl = document.getElementById('encoder-rate-control').value
      const targetBitrate =
      ...
      const fillerData = document.getElementById('encoder-filler-data-tgl')
        .checked
        ? 1
        : 0
      const multipass = document.getElementById('encoder-multipass').value
      ...
      emitUIInteraction({
        Console:
          'PixelStreaming.Encoder.TargetBitrate ' + targetBitrate > 0
            ? targetBitrate
            : -1,
      })
      emitUIInteraction({
        Console:
          'PixelStreaming.Encoder.MaxBitrateVBR ' + maxBitrate > 0
            ? maxBitrate
            : -1,
      })
  ...
  if (webrtcParamsSubmit !== null) {
    webrtcParamsSubmit.onclick = function (event) {
      const degradationPref = document.getElementById('webrtc-degradation-pref')
        .value
      const maxFPS = document.getElementById('webrtc-max-fps-text').value
      const minBitrate =
        document.getElementById('webrtc-min-bitrate-text').value * 1000
      const maxBitrate =
        document.getElementById('webrtc-max-bitrate-text').value * 1000
      const lowQP = document.getElementById('webrtc-low-qp-text').value
      const highQP = document.getElementById('webrtc-high-qp-text').value
  ...
  const matchViewportResolutionCheckBox = document.getElementById(
...
function setupWebRtcPlayer (htmlElement, config) {
  ...
  function showFreezeFrame () {
    ...
    const freezeFrameImage = document.getElementById('freezeFrameOverlay')
      .childNodes[0]
  ...
  webRtcPlayerObj.onDataChannelMessage = function (data) {
    ...
    if (view[0] === ToClientMessageType.QualityControlOwnership) {
      const ownership = view[1] === 0 ? false : true
      // console.log(
      ...
      if (settingsJSON.Encoder) {
        if (document.getElementById('encoder-rate-control')) {
          document.getElementById('encoder-rate-control').value =
            settingsJSON.Encoder.RateControl
        }
        if (document.getElementById('encoder-target-bitrate-text')) {
        ...
        if (document.getElementById('encoder-max-bitrate-text')) {
...
function onWebRtcAnswer (webRTCData) {
  ...
  webRtcPlayerObj.latencyTestTimings.OnAllLatencyTimingsReady = function (
    ...
    const latencyExcludingDecode =
      timings.BrowserReceiptTimeMs - timings.TestStartTimeMs
    const uePixelStreamLatency =
      timings.UEPreEncodeTimeMs == 0 || timings.UEPreCaptureTimeMs == 0
        ? '???'
        : timings.UEPostEncodeTimeMs - timings.UEPreCaptureTimeMs
    const captureLatency =
      timings.UEPostCaptureTimeMs - timings.UEPreCaptureTimeMs
    const encodeLatency = timings.UEPostEncodeTimeMs - timings.UEPreEncodeTimeMs
    const ueLatency = timings.UETransmissionTimeMs - timings.UEReceiptTimeMs
    const networkLatency = latencyExcludingDecode - ueLatency
    const browserSendLatency =
    ...
    latencyStatsInnerHTML +=
      timings.FrameDisplayDeltaTimeMs && timings.BrowserReceiptTimeMs
        ? `<div>Browser receive latency (ms): ${timings.FrameDisplayDeltaTimeMs}</div>`
        : ''
